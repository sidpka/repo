...
void PrintAncestors(struct TNode* root,int* Path,int key,int index){
...
PrintAncestors(root->left,Path,key,index);
PrintAncestors(root->right,Path,key,index);
}

}

int TestPrint(struct TNode* root,int key){
...
if(TestPrint(root->left,key) || TestPrint(root->right,key)){
printf("%d  ",root->data);
return 1;
}
...
printf("\nEnter key : ");
scanf("%d",&i);
printf("\nPress enter to print ancestors of : %d",i);
getch();
int* Path=(int*)malloc(sizeof(int)*n);
printf("\n\n");
//PrintAncestors(root,Path,i,0);
TestPrint(root,i);
getch();
return 0;
}
