...
int FindDiffSum(struct TNode* root){
...
for(i=1;i<=height;i++){
if(i%2){
oddSum+=GetSum(root,i);
}else{
evenSum+=GetSum(root,i);
}
}
...
getch();
printf("\n\nPress enter to see difference in sum of odd and even levels :\n\n");
getch();
printf("\nDiff sum : %d",FindDiffSum(root));
getch();
return 0;
}
