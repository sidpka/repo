...
int CheckIfFoldable(TNode* root,TNode* root1){
...
if((root->left && root1->left) && (root->right && root1->right)){
   return CheckIfFoldable(root->left,root1->left) && CheckIfFoldable(root->right,root1->right);
}
else if((root->left && root1->left)){
return CheckIfFoldable(root->left,root1->left);
}else if((root->right && root1->right)){
CheckIfFoldable(root->right,root1->right);
}else if((!root->left && !root1->left) && (!root->right && !root1->right)){
...
getch();
int result;
result=CheckIfFoldable(root->left,root->right);
