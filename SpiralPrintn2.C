...
void SpiralLeveelOrder(struct TNode* root,int level,int itr){
    if(!root){
    return;
    }
    if(level==1){
    ...
    if(itr){
    SpiralLeveelOrder(root->left,level-1,itr);
    SpiralLeveelOrder(root->right,level-1,itr);
    }else{
    SpiralLeveelOrder(root->right,level-1,itr);
    SpiralLeveelOrder(root->left,level-1,itr);
...
void AnalysisSpiral(struct TNode* root){
int height=Height(root);
int i;
int itr=0;
for(i=1;i<=height;i++){
SpiralLeveelOrder(root,i,itr);
itr=!itr;
printf("\n\n");
}
printf("\n\n");
