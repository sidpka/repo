...
struct TNode* CreateTree(int* preorder,char* preorderLN,int* index,int n){

struct TNode* newNode=(struct TNode*)malloc(sizeof(struct TNode));
int tmpIndex=(*index);
newNode->data=preorder[(*index)++];
if(preorderLN[tmpIndex]=='N'){
newNode->left=CreateTree(preorder,preorderLN,index,n);
newNode->right=CreateTree(preorder,preorderLN,index,n);
}else{
newNode->left=NULL;
newNode->right=NULL;
}
...
char* preorderLN=(char*)malloc(sizeof(char)*n);
printf("\n\nInorder sequence taken\n\n");
getch();
printf("\nEnter preOrder LN sequence..\\n\n");
scanf("%s",preorderLN);
printf("\nSequence : %s",preorderLN);
printf("\n\npreorderLN sequence taken..\n\n");
getch();
printf("\nPress enter to create tree");
int index=0;
root=CreateTree(preorder,preorderLN,&index,n);
