...
void CreateBST(TNode** root,int* buffer,int* i){
if(!(*root)){
return;
}

CreateBST(&(*root)->left,buffer,i);
(*root)->data=buffer[(*i)++];
CreateBST(&(*root)->right,buffer,i);
...
int ind=0;
PickNodes(root,buffer,&ind);
printf("\nPress enter to sort the array\n");
qsort(buffer,n,sizeof(int),cmp);
for(i=0;i<n;i++){
printf("%d  ",buffer[i]);
}

printf("\nSorted\nNow press enter to Create BST\n");
ind=0;
CreateBST(&root,buffer,&ind);
