...
void PrintNodesBetweenLevels(struct TNode* root,int a,int b,int level){
if(!root){
return;
}

PrintNodesBetweenLevels(root->left,a,b,level+1);

if(level>=a && level <=b){
printf("%d  ",root->data);
}

PrintNodesBetweenLevels(root->right,a,b,level+1);
...
int a,b;
scanf("%d %d",&a,&b);
PrintNodesBetweenLevels(root,a,b,1);
getch();
