...
struct TNode* PrintNextRight(struct TNode* root,struct TNode* findNode,int level,int* findLevel){
if(!root){
return NULL;
}
...
struct TNode* resultLeft=PrintNextRight(root->left,findNode,level+1,findLevel);
struct TNode* resultRight=PrintNextRight(root->right,findNode,level+1,findLevel);
if(resultLeft!=NULL){
return resultLeft;
}else{
return resultRight;
...
int level=-1;
int result=0;
struct TNode* tmpNode=PrintNextRight(root,root->left->right,0,&level);
if(tmpNode){
result=tmpNode->data;
}
printf("\nNext right of : %d is %d ",root->left->right->data,result);
return 0;
}
