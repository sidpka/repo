...
struct TNode* ConvertLeftRightToDownRight(struct TNode** root,int n){
if(!root){
return NULL;
}
...
struct TNode* tmpNode;
int nodeCount;
struct TNode* firstNode=*root;
while(1){
nodeCount=QueueSize(queue);
if(nodeCount<=0){
break;
}
...
void TraverseDownRight(struct TNode* root){
if(!root){
return;
}
printf("%d  ",root->data);
TraverseDownRight(root->right);
TraverseDownRight(root->left);
}
...
getch();
root=ConvertLeftRightToDownRight(&root,n);
printf("\nTraverse downRight");
TraverseDownRight(root);
return 0;
}
