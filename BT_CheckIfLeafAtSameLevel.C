...
int CheckIfLeafAtSameLevel(struct TNode* root,int level,int* lookup){
if(!root){
return 1;
}
...
int right=1;
if(root->left){
left=CheckIfLeafAtSameLevel(root->left,level+1,lookup);
}
if(root->right){
right=CheckIfLeafAtSameLevel(root->right,level+1,lookup);
}
return left&right;
}
...
getch();
printf("\nPress enter to see if all nodes are at same level\n");
getch();
int lookUp=0;
int result=CheckIfLeafAtSameLevel(root,1,&lookUp);
if(result){
printf("\nYes");
}else{
printf("\nNo");
