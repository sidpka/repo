...
int k;
scanf("%d",&k);
BuildHeap(heap,Array,k);
printf("\nPress enter to get min %d elements\n",k);
getch();
GetMinK(heap,Array,n,k);
printf("\nPress enter to get smallest %d  elements\n",k);
getch();
for(i=0;i<heap->count;i++){
printf("%d  ",heap->Array[i]);
}
getch();
printf("\nPress enter to get sorted samleest elements\n");
getch();
HeapSort(heap);
for(i=0;i<heap->count;i++){
printf("%d  ",heap->Array[i]);
