...
void LeftDeepestLeaf(struct TNode* root,struct TNode** leftLeaf,int* leafLevel,int level){
...
if(root->left){
LeftDeepestLeaf(root->left,&(*leftLeaf),leafLevel,level+1);
}
if(root->right){
LeftDeepestLeaf(root->right,&(*leftLeaf),leafLevel,level+1);
}
...
getch();
printf("\nPress enter to see deepest left leaf node data   :   ");
getch();
// for testing
...
tmpNode->left=NULL;
tmpNode->right=NULL;
root->left->right->left=tmpNode;
//end of testing
int leafLevel=0;
struct TNode* leafNode=NULL;
LeftDeepestLeaf(root,&leafNode,&leafLevel,0);
printf("\nHere we go : %d",leafNode->data);
getch();
return 0;
}
