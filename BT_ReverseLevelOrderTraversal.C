...
void VisitNodes(struct TNode* root,int depth){
...
if(depth>1){
VisitNodes(root->left,depth-1);
VisitNodes(root->right,depth-1);
}

}


void ReverseLevelOrder(struct TNode* root){
...
getch();
printf("\nPress enter n see reverse level order taversal\n\n");
getch();
