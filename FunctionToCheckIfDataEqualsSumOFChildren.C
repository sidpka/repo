...
/*
int CheckIfTrue(struct TNode* root){
if(!root){
return 0;
}

int left=CheckIfTrue(root->left);
int right=CheckIfTrue(root->right);
...
left=CheckIfTrue(root->left);
right=CheckIfTrue(root->right);
if(root->data==root->left->data+root->right->data && left && right){
return 1;
}else{
return 0;
}
...
getch();
printf("\nPress enter to see if all the nodes have data equal to sum of data of their children : ");
int result=CheckIfTrue(root);
if(result){
printf("\nSatified");
}else{
printf("\nNo");
