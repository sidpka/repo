...
int PrintNodesAt(struct TNode* root,int level,int key){
if(!root){
return 0;
}
if(level==1){
...
if(level>1){
return PrintNodesAt(root->left,level-1,key) || PrintNodesAt(root->right,level-1,key);
}
}



int LevelOfKey(struct TNode* root,int key){
if(!root){
return 0;
}
...
printf("\nEnter key : ");
scanf("%d",&i);
printf("\nPress enter to see level of : %d",i);
getch();
printf("\n\n");
i=LevelOfKey(root,i);
if(i){
