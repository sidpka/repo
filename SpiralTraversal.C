...
SNode* pushStack=NULL;
SNode* popStack=NULL;
Push(&pushStack,root);
SNode* tmp;
printf("\n\n");
struct TNode* tmpNode;
while(!IsEmptyStack(pushStack) || !IsEmptyStack(popStack)){
    ...
    tmpNode=Pop(&popStack);
    printf("%d  ",tmpNode->data);
    if(level&1){
    if(tmpNode->right){
    Push(&pushStack,tmpNode->right);
    }
    if(tmpNode->left){
    Push(&pushStack,tmpNode->left);
    }
    }
    else{
     if(tmpNode->left){
