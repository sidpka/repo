...
void DFS(int** array,int* visited,int s ,int n){

visited[s]=1;
int i;

for(i=0;i<n;i++){
if(visited[i]==0 && array[s][i]!=0){
...
void FordFulkerson(int** array,int s,int t,int n){
...
DFS(rGraph,visited,s,n);


for(i=0;i<n;i++){
for(j=0;j<n;j++){
if(visited[i] && !visited[j] && array[i][j]){
printf("%d --- %d",i,j);
}
}
}
