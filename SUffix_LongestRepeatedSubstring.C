...
void LCS_Util(STree* node,int* max,int* suffixInd,int label){
...
}else if(node->suffixIndex>-1 && (*max)<label-EdgeLength(node)){
(*max)=label-EdgeLength(node);
(*suffixInd)=node->suffixIndex;
}
}
...
//printf("%s",text);
SuffixTree(text);
printf("\nPress enter to find Longest Repeated substring\n\n");
getch();
LCS();
FreeSuffixTree(root);
return 0;
