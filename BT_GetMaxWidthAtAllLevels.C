...
void PopulateCount(struct TNode* root,int* count,int level){
    if(!root){
    return;
    }
count[level]++;
PopulateCount(root->left,count,level+1);
PopulateCount(root->right,count,level+1);
}
...
getch();
printf("\nMax Width is : %d",GetMaxWidth(root));
return 0;
}
