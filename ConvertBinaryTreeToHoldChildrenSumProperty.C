...
void ConvertChildreSumTrue(struct TNode** root){
    if(!(*root)){
    return;
    }
...
ConvertChildreSumTrue(&(*root)->left);
ConvertChildreSumTrue(&(*root)->right);
...
printf("\nPress enter to convert this tree to hold children sum property::\n\n");
getch();
ConvertChildreSumTrue(&root);
LevelOrderTraversal(root,n);
getch();
