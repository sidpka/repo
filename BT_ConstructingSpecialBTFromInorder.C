...
int FindIndexOfMaxNode(int* inorder,int startIndex,int endIndex){
int i;
int max=0;
int maxIndex;
...
struct TNode* CreateTree(int* inorder,int startIndex,int endIndex){
if(startIndex>endIndex){
return NULL;
}
int currentNodeIndex=FindIndexOfMaxNode(inorder,startIndex,endIndex);
struct TNode* newNode=(struct TNode*)malloc(sizeof(struct TNode));
newNode->data=inorder[currentNodeIndex];
newNode->left=CreateTree(inorder,startIndex,currentNodeIndex-1);
newNode->right=CreateTree(inorder,currentNodeIndex+1,endIndex);
...
printf("\n\nInorder sequence taken\n\n");
getch();
printf("\nPress enter to create tree");
root=CreateTree(inorder,0,n-1);
