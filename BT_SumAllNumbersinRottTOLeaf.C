...
int GetNumberTopBottomManner(int* buffer,int in){
int i;
int number=0;
for(i=0;i<in;i++){
number=number*10;
...
void FindSum(struct TNode* root,int* buffer,int* sum,int index){
...
if(root->left){
FindSum(root->left,buffer,sum,index);
}

if(IsLeafNode(root)){
(*sum)+=GetNumberTopBottomManner(buffer,index);
return;
}
if(root->right){
FindSum(root->right,buffer,sum,index);
}
...
printf("\nPress enter to find sum of all the numbers from root to leaf paths\n\n");
getch();
FindSum(root,buffer,&sum,0);
printf("\n\nSum is : %d\n",sum);
getch();
return 0;
}
