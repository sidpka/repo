...
void PrintNodesAt(struct TNode* root,int level){
if(!root){
return;
}
if(level==1){
...
if(level>1){
PrintNodesAt(root->left,level-1);
PrintNodesAt(root->right,level-1);
}
}

void PrintNodesAtDistanceK(struct TNode* root,int distance){


//int height=Height(root);

PrintNodesAt(root,distance+1);
...
printf("\nEnter distance : ");
scanf("%d",&i);
printf("\nPress enter to print all nodes at distance : %d",i);
getch();
printf("\n\n");
