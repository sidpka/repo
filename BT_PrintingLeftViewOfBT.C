...
void LeftView(struct TNode* root,int level,int *flag){
    if((*flag)){
    return;
    }
if(!root){
...
if(level>1){
if(root->left){
LeftView(root->left,level-1,flag);
}
if(root->right){
LeftView(root->right,level-1,flag);
}
}
}
void PrintLeftView(struct TNode* root){
...
getch();
printf("\n\nPress enter to print left view:\n\n");
getch();
PrintLeftView(root);
getch();
return 0;
