...
int PairWithSumExists(HNode* hash,int* Array,int sum,int n){

int i;

for(i=0;i<n;i++){
if(Search(hash,sum-Array[i])){
return 1;
}
}
...
int sum;
getch();
printf("\nEnter the sum you want to see if a pair of elements in array sum up to \n");
scanf("%d",&sum);

int result=PairWithSumExists(hash,Array,sum,m);

if(result){
printf("\nYes such pair exists\n");
}else{
printf("\No failed");
}
