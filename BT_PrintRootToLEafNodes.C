...
void RootToLeafPath(struct TNode* root,int* buffer,int index){
if(!root){
return;
}
...
RootToLeafPath(root->left,buffer,index);
RootToLeafPath(root->right,buffer,index);
}
}
