...
void BuildSuffixArray(STree* root,int* suffixArray,int* index){
if(!root){
return;
}
...
for(i=0;i<CHAR_MAX;i++){
if(root->child[i]!=NULL){
BuildSuffixArray(root->child[i],suffixArray,index);
}
}

}else if(root->suffixIndex>-1 && root->suffixIndex<size-1){
suffixArray[(*index)++]=root->suffixIndex;
}
...
memset(suffixArray,-1,sizeof(suffixArray));
int index=0;
SuffixTreeTraversal(root,level);

BuildSuffixArray(root,suffixArray,&index);
//free
