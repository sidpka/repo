#include<stdio.h>
#include<stdlib.h>
#include<conio.h>


struct TNode{

int data;
struct TNode* left;
struct TNode* right;
bool isRightThreaded;
bool isLeftThreaded;
};
//typedef struct TreeNode TNode;

struct QueueNode{
int capacity;
int front;
int rear;
struct TNode** Array;
};
typedef struct QueueNode QNode;

QNode* CreateQueue(int capacity){
QNode* queue=(QNode*)malloc(sizeof(QNode));
if(!queue){
printf("\nMemory Error\n\n");
}
queue->capacity=capacity;
queue->front=-1;
queue->rear=-1;
queue->Array=(struct TNode**)malloc(sizeof(struct TNode*)*queue->capacity);
int i;
for(i=0;i<queue->capacity;i++){
queue->Array[i]=(struct TNode*)malloc(sizeof(struct TNode));
}

return queue;
}

int IsEmptyQueue(QNode* queue){
return queue->front==-1;
}

int IsFullQueue(QNode* queue){
return (queue->rear+1)%queue->capacity==queue->front;
}

void ResizeQueue(QNode* queue){



queue->Array=(struct TNode**)realloc(queue->Array,queue->capacity*2);
int i;
for(i=queue->capacity;i<queue->capacity*2;i++){
queue->Array[i]=(struct TNode*)malloc(sizeof(struct TNode));
}
//printf("\nhere\n");
if(queue->front>queue->rear){
int i;

for(i=0;i<queue->front;i++){
queue->Array[i+queue->capacity]=queue->Array[i];
}
queue->rear=queue->capacity+i;
}
queue->capacity*=2;
}

void EnQueue(QNode* queue,struct TNode* node){

if((queue->rear+1)%queue->capacity==queue->front){
ResizeQueue(queue);
}
queue->rear=(queue->rear+1)%queue->capacity;
queue->Array[queue->rear]=node;
if(queue->front==-1){
queue->front=0;
}

}

TNode* DeQueue(QNode* queue){
if(IsEmptyQueue(queue)){
printf("\nNo data");
return NULL;
}else{struct TNode* data=queue->Array[queue->front];

if(queue->front==queue->rear){
queue->front=-1;
queue->rear=-1;
}else{
queue->front=(queue->front+1)%queue->capacity;
}

return data;
}
}

void DeleteQueue(QNode* queue){
int i;
for(i=0;i<queue->capacity;i++){
free(queue->Array[i]);
}
free(queue->Array);
free(queue);

}
void InsertBinaryTree(TNode** root,int data,int n){

if((*root)==NULL){
(*root)=(struct TNode*)malloc(sizeof(TNode));
(*root)->data=data;
(*root)->left=NULL;
(*root)->right=NULL;
(*root)->isRightThreaded=true;
(*root)->isLeftThreaded=true;
return;
}

QNode* queue=CreateQueue(n);
EnQueue(queue,(*root));
struct TNode* node=(struct TNode*)malloc(sizeof(TNode));
node->data=data;
node->isRightThreaded=true;
node->isLeftThreaded=true;
struct TNode* tmpNode;
while(!IsEmptyQueue(queue)){
tmpNode=DeQueue(queue);
if(tmpNode->isLeftThreaded){
node->right=tmpNode;
node->left=tmpNode->left;
tmpNode->isLeftThreaded=false;
tmpNode->left=node;
return;
}else{
EnQueue(queue,tmpNode->left);
}

if(tmpNode->isRightThreaded){
node->right=tmpNode->right;
node->left=tmpNode;
tmpNode->isRightThreaded=false;
tmpNode->right=node;
return;
}else{
EnQueue(queue,tmpNode->right);
}
}
}


void LevelOrderTraversalThreaded(struct TNode* root,int n){
if((root)==NULL){
return;
}

QNode* queue=CreateQueue(n);
EnQueue(queue,root);

struct TNode* tmpNode;
while(!IsEmptyQueue(queue)){
tmpNode=DeQueue(queue);

printf("%d  ",tmpNode->data);
if(!tmpNode->isLeftThreaded){
EnQueue(queue,tmpNode->left);
}

if(!tmpNode->isRightThreaded){
EnQueue(queue,tmpNode->right);
}


}
}

void InOrderTraversalThreaded(struct TNode* root){

if(!root){
return;
}
struct TNode* tmpNode=root;
while(tmpNode){

if(!tmpNode->isLeftThreaded){
while(!tmpNode->isLeftThreaded){
tmpNode=tmpNode->left;
}
}
while(tmpNode){
printf("%d  ",tmpNode->data);
tmpNode=tmpNode->right;
if(!tmpNode){
break;
}
if(tmpNode->isRightThreaded==false){
    printf("%d  ",tmpNode->data);
tmpNode=tmpNode->right;
break;
}
}

}
}

struct TNode* FindSuccessor(struct TNode* root){
    if(!root->right){
    return NULL;
    }
if(!root->isLeftThreaded){
return root->left;
}else{
struct TNode* tmpNode=root;
while(tmpNode->isRightThreaded){
tmpNode=tmpNode->right;
}
return tmpNode->right;
}
}

void PreOrderTraversalThreaded(struct TNode* root){
if(!root){
return;
}

struct TNode* tmpNode=(root);
int count=0;
while(tmpNode){

printf("%d  ",tmpNode->data);
tmpNode=FindSuccessor(tmpNode);

}

}

int main(){

struct TNode* root=NULL;
printf("\nEnter the number of elements you want to insert : ");
int n;
scanf("%d",&n);

int i,data;
for(i=0;i<n;i++){
printf("\nEnter data : ");
scanf("%d",&data);
InsertBinaryTree(&root,data,n);
}

printf("\nInserted... %d nodes \n",n);

getch();

printf("\nPress enter for level order traversal: \n\n");
LevelOrderTraversalThreaded(root,n);
getch();

printf("\nPress enter for inorder traversal : \n\n");
InOrderTraversalThreaded(root);
getch();
printf("\nPress enter for prIOrder traversal : \n\n");
PreOrderTraversalThreaded(root);
getch();
return 0;
}
