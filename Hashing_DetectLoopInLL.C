...
int CheckLoop(HNode* hashRoot,LNode* head){

LNode* tmpNode=head;

while(tmpNode){
if(Search(hashRoot,(int)tmpNode)){
return 1;
}
Insert(hashRoot,(int)tmpNode);
tmpNode=tmpNode->next;
}
return 0;
}
...
//printf("\n\nAddress of head : %d",head1);
//head1->next->next->next=head1->next;
int result=CheckLoop(hash,head1);
