...
void LOrderTraversal(struct TNode* root,int level){
if(!root){
return;
}
if(level==1){
...
if(level>1){
LOrderTraversal(root->left,level-1);
LOrderTraversal(root->right,level-1);
}
}

int LeafCount(struct TNode* node){
if(!node){
return 0;
}
...
void AssignTraversal(struct TNode* root){
