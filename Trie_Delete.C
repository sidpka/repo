...
void DeleteCurrent(TNode** root,char* data,int n,int currentLen,int maxPrefixLen){
if(!(*root)){
return;
}
if(currentLen<n){
int index=data[currentLen]-97;
DeleteCurrent(&(*root)->next[index],data,n,currentLen+1,maxPrefixLen);
}
if(currentLen>maxPrefixLen){
    //printf("\nHere");
free((*root));
(*root)=NULL;
}
...
int DeleteWord(TNode** root,char* data){
if(!(*root)){
return 0;
}
...
for(i=0;i<len;i++){
index=data[i]-97;
if(!tmpNode->next[index]){
return 0;
}
if(tmpNode->next[index]->data==1){
maxPrefixLength=i;
}
tmpNode=tmpNode->next[index];
}
...
return 1;
    ...
    /*
for(i=len-1;i>maxPrefixLength;i--){
index=data[i]-97;
free()
}
*/

DeleteCurrent(&(*root),data,len,0,maxPrefixLength);
return 1;
...
printf("\nPress enter to confirm whether %s id deleted",data);
result=SearchTrie(root,data);
getch();
if(result){
printf("\nSome Error ! Word could not be deleted ");
}else{
printf("\nYes Word has been deleted!! Confirmed!!!!");
}
getch();
printf("\nPress enter to scan the dictionary again : \n");
getch();
PrintAll(root,buffer,0);
return 0;
}
