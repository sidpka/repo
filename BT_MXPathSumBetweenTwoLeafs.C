...
int MaxSum(struct TNode* root,struct TNode* leftLeaf,struct TNode* rightLeaf,int* isLeftSet,int* isRightSet){

if(!root){
return -1;
}
if(root->left==NULL && root->right==NULL && (root==leftLeaf || root==rightLeaf)){
    if(root==leftLeaf){
    (*isLeftSet)=1;
    }else{
    (*isRightSet)=1;
...
int lResult=MaxSum(root->left,leftLeaf,rightLeaf,isLeftSet,isRightSet);
int rResult=MaxSum(root->right,leftLeaf,rightLeaf,isLeftSet,isRightSet);

if(lResult!=-1 && rResult!=-1){
return lResult+rResult+root->data;
}
else if(lResult!=-1){
...
return lResult;
    ...
    return lResult+root->data;
    }
}else if(rResult!=-1){
    if(*isLeftSet){
return rResult;
    }else{
    return rResult+root->data;
    }
}else{
return -1;
}
...
getch();
struct TNode* firstLeaf=root->right;
struct TNode* secondLeaf=root->left->right;
int isLeftSet=0;
int isRightSet=0;
int result=MaxSum(root,firstLeaf,secondLeaf,&isLeftSet,&isRightSet);
if(result!=-1){
printf("\nMax Sum Path : %d",result);
}else{
printf("\nNo path");
}
return 0;
