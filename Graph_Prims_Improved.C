...
void DijkastraUtil(GNode* graph,int* distance){
//printf("\n234");
...
if(IsInQueue(pQueue,tmpLink->destination) && distance[tmpLink->destination]> tmpLink->wait){
distance[tmpLink->destination]=tmpLink->wait;
UpdateWeight(pQueue,tmpLink->destination,tmpLink->wait);
parent[tmpLink->destination]=tmpNode->vertex;
}
