...
int CheckRootToLeafPathEqualsSum(struct TNode* root,int sum){
    int left,right;
...
right=CheckRootToLeafPathEqualsSum(root->right,sum-root->data);
if(right){
return 1;
}else{
return 0;
...
left=CheckRootToLeafPathEqualsSum(root->left,sum-root->data);
if(left){
return 1;
}else{
return 0;
